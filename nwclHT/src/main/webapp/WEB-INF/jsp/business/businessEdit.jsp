...
<script type="text/javascript">
	$(function() {
		...
		parent.$.messager.progress('close');
		$('#form').form({
			url : '${basePath}/web/business/edit',
			onSubmit : function() {
				editor.sync();
	...
	function deleteImg(id){
		parent.$.messager.confirm('询问', '您是否要删除当前图片？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/business/deleteImg', {
						id : id
					}, function(result) {
						if (result.code=="SUCCESS") {
							$('.item'+id).remove();
			...
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<td><input name="phone" type="text" style="width: 300px;"
						placeholder="" class="easyui-validatebox span2"
						data-options="required:true" value="${model.phone }"></td>
				</tr>
				<tr>
					...
					<div>
						<div>
							...
							<ul class="page_imgs clearfix" id="localPic"
								...
								<li> <img alt="" src="${model.imgPath }" style="height: 97px";width="116px"> </li>
								<li><input type="button" value="上传图片" style="margin-left: 10px;" id="doUpPic"></li>
							</ul>
						</div>
					</div>
					</td>
							...
							<ul style="list-style-type: none">
								<c:forEach items="${model.images }" var="list">
									<li style='float:left;margin:10px' class="item${list.id } span_div">
										...
										<span onclick="deleteImg(${list.id })" class="close_span"><img class="dimg" alt=""  src="/nwclHT/images/delete.png"></span>
									</li>
								</c:forEach>
							</ul>
						</div>
