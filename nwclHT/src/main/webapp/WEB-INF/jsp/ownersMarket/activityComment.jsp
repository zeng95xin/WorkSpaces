   ...
   <script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${basePath}/web/OwnersMarketWebController/activityCommentDataGrid?id='+${messageId},
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
...
/* 			frozenColumns : [ [  ] ], */
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/OwnersMarketWebController/deleteComment', {
						id : id
					}, function(result) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${basePath}/web/OwnersMarketWebController/batchDeleteComment', {
						ids : ids.join(',')
					}, function(result) {
						if (result.code=="SUCCESS") {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 650,
			height : 400,
			href : '${basePath}/web/OwnersMarketWebController/activityCommentEditPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
