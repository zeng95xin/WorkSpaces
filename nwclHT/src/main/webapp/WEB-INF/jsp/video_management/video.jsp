...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/VideoManagement/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
		...
		},
		{
			...
			formatter : function(value, row, index) {
				...
				str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				str += $.formatString('<img onclick="authFun(\'{0}\');" src="{1}" title="授权"/>', row.id, '${basePath}/style/images/extjs_icons/key.png');
				str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 350,
		height : 300,
		href : '${basePath}/web/VideoManagement/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 350,
		height : 300,
		href : '${basePath}/web/VideoManagement/editPage?id=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function authFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 500,
		height : 500,
		href : '${basePath}/web/VideoManagement/grantPage?id=' + id,
		buttons : [ {
			text : '授权',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除视频信息 ', function(b) {
		if (b) {
			...
			if (currentUserId) {
				...
				$.post('${basePath}/web/VideoManagement/delete', {
					id : id
				}, function(result) {
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="width:900px;">
					<tr>
						<th>视频名称:</th>
						<td>
						<input type="text" name="name" id="name" placeholder="请输入视频名称">
						</td>
						<td>
