...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/building/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
...
function  addFun(){
	parent.$.messager.confirm('询问', '您是否确定添加楼栋信息，添加完成后一旦使用将不可删除，只能修改楼栋信息！ ', function(b) {
		if(b){
			parent.$.messager.progress('close');
			parent.$.modalDialog({
				title : '添加',
				width : 350,
				height : 400,
				href : '${basePath}/web/building/addPage',
				buttons : [{
						text : '添加',
						handler : function(){
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 400,
			height : 300,
			href : '${basePath}/web/building/editPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除楼栋信息 ', function(b) {
		if (b) {
				parent.$.messager.progress({
					title : '提示',
					text : '数据处理中，请稍后....'
				});
				$.post('${basePath}/web/building/delete', {
					id : id
				}, function(result) {
				...
				<table class="table table-hover table-condensed" style="width:900px;">
					<tr>
						...
						<td><select name="name" class=" span2" >
							<option value="">--请选择--</option>
							<option value="name">楼宇名称</option>
							<option value="address">楼宇地址</option>
