 ...
 */
	...
	$(function() {
		...
		$('#form').form({
			url : '${basePath}/web/NeighborHelpWebController/edit',
			onSubmit : function() {
	...
	function deleteImg(id){
		parent.$.messager.confirm('询问', '您是否要删除当前图片？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/NeighborHelpWebController/deleteImg', {
						id : id
					}, function(result) {
						if (result.code=="SUCCESS") {
							$('.item'+id).remove();
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="">
		<form id="form" method="post"  encType="multipart/form-data" >
				<input type="hidden" value="${neighborHelp.id }" name="id">
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<td colspan="3" >
					<input type="text" name="title" value="${neighborHelp.title }" class="easyui-validatebox span2" 
						data-options="required:true" >
					</td>
					<th>悬赏</th>
					<td>
					<input type="text" name="reward" value="${neighborHelp.reward }" class="easyui-validatebox span2" 
						data-options="required:true" >
					</td>
				...
				<tr>
					...
					<td colspan="5" style="height: 200px;">
						<textarea name="content" id="activity"  style="width:500px;height: 200px;">${neighborHelp.content }</textarea>
				</tr>
				<tr>
					<th style="text-align: right; width: 40px;">图片</th>
					<td colspan="4" class="text-box">
					<c:forEach items="${neighborHelpImg }" var="list">
					<div id="div_span">
