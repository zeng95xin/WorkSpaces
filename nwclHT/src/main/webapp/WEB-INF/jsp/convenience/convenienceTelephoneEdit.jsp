...
<script type="text/javascript">
	$(function() {
		...
		$('#form').form({
			url : '${basePath}/web/convenienceTelephone/edit',
			onSubmit : function() {
					...
					if(data.code == "SUCCESS"){
						if(data.result != null){
							...
							for (var i = 0; i < data.result.length; i++) {
								sss += "<option value="+data.result[i].id;
								if(data.result[i].id == ${c.type}){
									sss += " selected='selected' ";
								}
								sss += ">"+data.result[i].name+"</option>";
							}
							$("#type").html(sss);
						}
					}
			  ...
			  <tr>		
				...
				<tr>
					...
					<td>
						...
						data-options="required:true" >
							...
							<c:forEach items="${comus }" var="co">
								<option
								<c:if test="${co.id == c.communityId }">selected="selected"</c:if>
								 value="${co.id }"
				...
				<tr>
					...
					<td><input name="telephone" type="text" style="width: 210px;"
						...
						data-options="required:true" value="${c.telephone}"></td>
				</tr>
