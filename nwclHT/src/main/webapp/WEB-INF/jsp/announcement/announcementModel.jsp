...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/announcementModel/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 650,
		height : 650,
		href : '${basePath}/web/announcementModel/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 650,
		height : 650,
		href : '${basePath}/web/announcementModel/editPage?id=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	parent.$.messager.confirm('询问', '您是否要删除通知信息 ', function(b) {
		if (b) {
			...
			if (currentUserId != id) {
				...
				$.post('${basePath}/web/announcementModel/delete', {
					id : id
				}, function(result) {
