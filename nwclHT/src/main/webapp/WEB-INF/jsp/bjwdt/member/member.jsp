...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/bjwdt/member/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
		...
		nowrap : false,
		columns : [ [ 
			{field : 'loginName', title : '登录名', width : 150, align : 'center', sortable : false }
			,{field : 'nickName', title : '昵称', width : 150, align : 'center', sortable : false }
			,{field : 'createTime', title : '创建时间', width : 150, align : 'center', sortable : false }
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 600,
		height : 600,
		href : '${basePath}/web/bjwdt/member/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 600,
		height :300,
		href : '${basePath}/web/bjwdt/member/editPage?code=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除 ', function(b) {
		if (b) {
			...
			$.post('${basePath}/web/bjwdt/member/delete', {
				code : id
			}, function(result) {
