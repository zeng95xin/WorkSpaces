   ...
   <script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${basePath}/web/ActivityRegisterWebController/dataGrid?id='+${id},
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/ActivityRegisterWebController/delete', {
						id : id
					}, function(result) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${basePath}/web/ActivityRegisterWebController/batchDelete', {
						ids : ids.join(',')
					}, function(result) {
						if (result.code=="SUCCESS") {
