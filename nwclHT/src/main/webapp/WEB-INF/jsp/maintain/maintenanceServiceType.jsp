...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/maintenanceServiceType/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
		...
		},
		{
			...
			formatter : function(value, row, index) {
				...
				str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			    str += $.formatString('<img onclick="classifyFun(\'{0}\');" src="{1}" title="子项目"/>', row.id, '${basePath}/style/images/extjs_icons/style_edit.png');
				str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 350,
		height : 300,
		href : '${basePath}/web/maintenanceServiceType/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 350,
		height : 300,
		href : '${basePath}/web/maintenanceServiceType/editPage?id=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function  classifyFun(id){
	window.open ('${basePath}/web/maintenanceServiceType/classifyPage?type='+id,'分类','height=500,width=800,top=100,left=100,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') ;
}
	...
	parent.$.messager.confirm('询问', '您是否要删除信息 ', function(b) {
		if (b) {
			...
			if (currentUserId ) {
				...
				$.post('${basePath}/web/maintenanceServiceType/delete', {
					id : id
				}, function(result) {
