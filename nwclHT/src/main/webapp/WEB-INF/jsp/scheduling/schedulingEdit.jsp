...
<script type="text/javascript">
	$(function() {
		...
		$('#form').form({
			url : '${basePath}web/scheduling/edit?id=${s.id}',
			onSubmit : function() {
...
function typeSelect(a){
	...
	communityId = $("#Com").val();
	$.post('${basePath}web/scheduling/getSuser',{type : a, communityId : communityId},
			 function(data){
			   if (data.length > 0) {
				var str = "";
...
function communitySelect(a){
	...
	$.post('${basePath}web/scheduling/getBuilding',{communityId : a},
			 function(data){
			   if (data.length > 0) {
		...
		<form id="form" method="post">
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<td>
						<select id="type" name="type" style="width: 200px;" data-options="required:true" onchange="typeSelect(this.value)">
							<option <c:if test="${type ==1 }">selected="selected"</c:if> value="1">管家</option>
							<option <c:if test="${type ==2 }">selected="selected"</c:if> value="2">保安</option>
							<option <c:if test="${type ==3 }">selected="selected"</c:if> value="3">维修人员</option>
						</select>
					</td>
					<th style="text-align: right; width: 100px;">值班人员</th>
					<td>
						<select id="suserId" name="suserId" style="width: 200px;">
						<c:forEach items="${list_su }" var="suser">
							<option
							<c:if test="${suser.id == s.suserId }">selected</c:if> value="${suser.id }">${suser.name }</option>
						</c:forEach>
						</select>
					</td>						
				</tr>
				<tr>
					...
					<td>
						<input class="easyui-datetimebox" name="startTime" value="${startTime }" data-options="showSeconds:false"style="width:200px"/>
					</td>
					<th style="text-align: right; width: 100px;">结束时间</th>
					<td>
						<input class="easyui-datetimebox" name="endTime" value="${endTime }" data-options="showSeconds:false"style="width:200px"/>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3" id="buildingId">
							<c:if test="${type == 1 }">
								<span style="display: none;" id="all"><input onclick="checkboxAllfun()" id="buildingAll"  type="checkbox" value="-1"/>所有楼栋</span><p>
							</c:if>
							<c:if test="${type != 1 }">
								<span style="display: block;" id="all"><input onclick="checkboxAllfun()" id="buildingAll"  type="checkbox" value="-1"/>所有楼栋</span>
							</c:if>
							<c:forEach items="${list_building }" var="list">
								<input type="checkbox" <c:if test="${building.contains(list.id.toString()) }">checked="checked"</c:if> name='building' value="${list.id }">${list.name }&nbsp;&nbsp;&nbsp;&nbsp;
							</c:forEach>
					</td>
				</tr>
