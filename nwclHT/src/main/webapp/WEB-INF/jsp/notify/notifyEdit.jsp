...
<!-- 引入kindEditor插件 -->
<link rel="stylesheet"
	href="${basePath}/js/kindeditor-4.1.7/themes/default/default.css">
<script type="text/javascript"
	src="${basePath}/js/kindeditor-4.1.7/kindeditor-all-min.js"
	charset="utf-8"></script>
<script type="text/javascript">
	$(function() {
	...
	function setImagePreview() {
		...
		for (var i = 0; i < fileList.length; i++) {
			...
			$("#localImag")
					.prepend(
							"<li style='float:left;margin-right:15px' > <img id='preview" + i + "'  /> </li>");
	...
	function deleteImg(id) {
		parent.$.messager.confirm('询问', '您是否要删除当前图片？', function(b) {
			if (b) {
				...
				$.post('${basePath}/web/notify/deleteImg', {
					id : id
				}, function(result) {
					if (result.code == "SUCCESS") {
						$('.item' + id).remove();
	...
	<div data-options="region:'center',border:false" title="">
		<form id="form" method="post" encType="multipart/form-data">
			...
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<td rowspan="7" style="width: 250px;">
						<div style="width:90%;float:right">
							...
							<br /> <br /> <input type="button" value="确认选择"
								onclick="getNames()">
						</div>
				...
				<tr>
					...
					<td><select name="type" >
							<option value="${model.type }">
								<c:if test="${model.type eq -1 }">管家消息</c:if>
								<c:if test="${model.type eq 2 }">借用单消息</c:if>
								<c:if test="${model.type eq 3 }">挂画单消息</c:if>
								<c:if test="${model.type eq 4 }">维修单消息</c:if>
								<c:if test="${model.type eq 5 }">访客消息</c:if>
								<c:if test="${model.type eq 6 }">放行条消息</c:if>
								<c:if test="${model.type eq 7 }">授权消息</c:if>
								<c:if test="${model.type eq 9 }">邻里帮消息</c:if>
								<c:if test="${model.type eq 10 }">公告消息</c:if>
								<c:if test="${model.type eq 11 }">业主讲堂消息</c:if>
								<c:if test="${model.type eq 12 }">业主市集消息</c:if>
							</option>
							...
							<ul style="list-style-type: none">
								<c:forEach items="${model.images }" var="list">
									<li style='float:left;margin:10px'
										class="item${list.id } span_div"><img class="hp" alt=""
										src="${list.imgPath }" style="height: 97px" ;width="116px">
										<span onclick="deleteImg(${list.id })" class="close_span"><img
											class="dimg" alt="" src="/images/delete.png"></span></li>
								</c:forEach>
							</ul>
						</div>
					</td>
				...
				<tr>
					<th style="text-align: right; width: 50px;">添加图片</th>
				...
				<tr>
					...
					<td><textarea name="detail" style="width:300px;height: 100px;"
							id="Content" class="easyui-validatebox span2">${model.detail }</textarea></td>
				</tr>
				<tr>
					<th style="text-align: right; width: 50px;">备注</th>
					<td><textarea name="remark" style="width:455px;height: 190px;"
