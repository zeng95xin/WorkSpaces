package com.bola.nwcl.web.kindEditorFile;
...
import com.bola.nwcl.web.comparator.NameComparator;
import com.bola.nwcl.web.comparator.SizeComparator;
import com.bola.nwcl.web.comparator.TypeComparator;
...
public class FileController extends BaseController {
	...
	public Map<String, Object> fileManage(HttpServletResponse response, HttpServletRequest request, HttpSession session) {
		...
		String rootPath = Configure.getRootPath() + "/upload/file/attached/";

		// 根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
		String rootUrl = Configure.getRootUrl();
	...
	@ResponseBody
	public Map<String, Object> upload(HttpServletResponse response, HttpServletRequest request,MultipartFile imgFile, HttpSession session) {
		...
		m.put("message", "上传失败！");
		// 文件保存目录路径
		String savePath = Configure.getRootPath()+"/upload/file/attached/";

		// 文件保存目录URL
		String saveUrl = "/upload/file/attached/";
		...
		long maxSize = Long.parseLong("10240000"); // 允许上传最大文件大小(字节)
		...
		if (ServletFileUpload.isMultipartContent(request)) {// 判断表单是否存在enctype="multipart/form-data"
			...
			try {
				...
				m.put("error", 0);
				m.put("url", rootUrl+saveUrl + newFileName);
