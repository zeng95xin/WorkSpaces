...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#form').form({
			url : '${basePath}/web/QuestionWebController/edit',
			onSubmit : function() {
			...
			<table class="table table-hover table-condensed">
				<tr>
					...
					<td><select name="type" id="selectbox" onchange="selectchange()" style="width: 100px">
						...
						<option value="3" <c:if test="${question.type==3 }">selected</c:if> >填空题</option>
						</select></td>
						
					<th>题目</th>
					<td>
					<input type="text" name="title" class="easyui-validatebox span2" 
						data-options="required:true" value="${question.content }">
					</td>	
				</tr>
				<tr>
				<th colspan="4">选项</th>
				...
				<div class="text-box">
					...
					<c:forEach items="${questionItem}" var="list">
					<div class="item"><span class="lspan">4:</span> <input name="content" value="${list.content }" type="text" class="input_text"><span class="box-delete">×</span></div>
					</c:forEach>
				</div>
				<c:if test="${question.type==3 }">
				<div class="" style="display:none">
					<div class="item"><span class="lspan">1:</span> <input type="text" name="contentsss" value="${list.content }" class="input_text"></div>
				</div>
				</c:if>
