   ...
   <script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${basePath}/web/scheduling/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
...
/* 			frozenColumns : [ [  ] ], */
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/scheduling/delete', {
						id : id
					}, function(result) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${basePath}/web/scheduling/batchDelete', {
						ids : ids.join(',')
					}, function(result) {
						if (result.code=="SUCCESS") {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 700,
			height : 400,
			href : '${basePath}/web/scheduling/editPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 700,
			height : 400,
			href : '${basePath}/web/scheduling/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="width: 70%">
					<tr>
						<td>小区</td>
						<td>
						<select name="communityId" style="width: 100px" class="easyui-combobox" >
