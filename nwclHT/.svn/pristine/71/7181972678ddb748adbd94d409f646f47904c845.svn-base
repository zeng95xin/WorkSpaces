...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/department/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
		...
		},		
		{
			...
			formatter : function(value, row, index) {
				var str = '&nbsp;&nbsp;&nbsp;&nbsp;';
				if (${loginUser.roleId == 1} && row.id != 1 && row.id != 2 && row.id != 3) {
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 350,
		height : 350,
		href : '${basePath}/web/department/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 350,
		height : 350,
		href : '${basePath}/web/department/editPage?id=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	parent.$.messager.confirm('询问', '您是否要删除部门信息 ', function(b) {
		if (b) {
			...
			if (currentUserId) {
				...
				$.post('${basePath}/web/department/delete', {
					id : id
				}, function(result) {
