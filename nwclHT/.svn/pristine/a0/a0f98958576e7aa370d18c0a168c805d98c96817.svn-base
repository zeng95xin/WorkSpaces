 ...
 */
	...
	$(function() {
		...
		$('#form').form({
			url : '${basePath}/web/LikeShareWebController/edit',
			onSubmit : function() {
	...
	function deleteImg(id){
		parent.$.messager.confirm('询问', '您是否要删除当前图片？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/LikeShareWebController/deleteImg', {
						id : id
					}, function(result) {
						if (result.code=="SUCCESS") {
							$('.item'+id).remove();
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="">
		<form id="form" method="post"  encType="multipart/form-data" >
				<input type="hidden" value="${likeShare.id }" name="id">
			<table class="table table-hover table-condensed">
				...
				<tr>
					...
					<td colspan = "3">
					<input type="text" name="title" value="${likeShare.title }" class="easyui-validatebox span2" 
						data-options="required:true" >
					</td>
					<th>分类</th>
					<td>
					<select name="type" id="selectbox" onchange="selectchange()" style="width: 100px">
						<option value="-1">--请选择--</option>
						<option value="1" <c:if test="${likeShare.shareType=='精彩活动' }">selected</c:if> >精彩活动</option>
						<option value="2" <c:if test="${likeShare.shareType=='和谐社区' }">selected</c:if> >和谐社区</option>
						<option value="3" <c:if test="${likeShare.shareType=='生活好物' }">selected</c:if> >生活好物</option>
						</select></td>
				</tr>
				<tr>
					<th style="text-align: right; width: 40px;">内容</th>
					<td colspan="5" style="height: 200px;">
						<textarea name="content" id="activity"  style="width:500px;height: 200px;">${likeShare.content }</textarea>
				</tr>
				<tr>
					<th style="text-align: right; width: 40px;">图片</th>
					<td colspan="4" class="text-box">
					<c:forEach items="${likeShareImg }" var="list">
					<div id="div_span">
