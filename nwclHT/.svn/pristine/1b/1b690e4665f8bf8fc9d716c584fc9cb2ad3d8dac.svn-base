   ...
   <script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${basePath}/web/OwnersMarketWebController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
...
/* 			frozenColumns : [ [  ] ], */
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 600,
			href : '${basePath}/web/OwnersMarketWebController/editPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
	...
	function commentFun(id) {
		window.open(
			'${basePath}/web/OwnersMarketWebController/commentPage?id=' + id, window,
			"dialogHeight:500px;dialogWidth:1000px;status:no;");
	}
	
	function readingFun(id) {
		window.open(
			'${basePath}/web/readingRecord/manager?contentId=' + id+'&type=6', window,
