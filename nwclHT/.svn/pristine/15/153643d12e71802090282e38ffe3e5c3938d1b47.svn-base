	...
	<div data-options="region:'center',border:false" title="">
		<form id="form" method="post">
			<table class="table table-hover table-condensed">
                <tr>					
					<td><input name="id" type="hidden" value="${c.id}"></td>
				</tr>
				<tr>
					...
					<td><input name="serial" type="text" style="width: 210px;"
						...
						readonly="readonly" value="${c.serial}"></td>
				</tr>
				<tr>
					...
					<td><input name="name" type="text" style="width: 210px;"
						...
						readonly="readonly" value="${c.name}"></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<option value="">--请选择物业--</option>
					   ...
					   <option value="${list.id }" <c:if test="${list.id==c.propertyId }">selected</c:if>>${list.name }</option>
					   </c:forEach>
					    </select>
					</td>
				</tr>
				<tr>
					<th style="text-align: right; width: 160px;">门禁类型</th>
					<td><select name="doorTypeId"  style="width: 210px;">
					   <c:forEach items="${doorTypes}" var="list">
					   <option value="${list.id }" <c:if test="${list.id==c.doorTypeId }">selected</c:if>>${list.name }</option>
					   </c:forEach>
					    </select>
					</td>
				</tr>
				<tr>
					...
					<textarea name="note"style="width: 210px;"
					    class="easyui-validatebox span2" >${c.note}</textarea></td>
				</tr>
