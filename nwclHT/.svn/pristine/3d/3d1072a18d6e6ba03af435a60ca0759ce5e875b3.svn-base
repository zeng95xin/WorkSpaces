   ...
   <script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${basePath}/web/QuestionWebController/questionDataGrid?questionId='+${questionId},
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
...
/* 			frozenColumns : [ [  ] ], */
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
				...
				/* if (currentUserId != id) { */
					...
					$.post('${basePath}/web/QuestionWebController/delete', {
						id : id
					}, function(result) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${basePath}/web/QuestionWebController/batchDelete', {
						ids : ids.join(',')
					}, function(result) {
						if (result.code=="SUCCESS") {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 800,
			height : 700,
			href : '${basePath}/web/QuestionWebController/addPage?questionId='+${questionId},
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 700,
			height : 300,
			href : '${basePath}/web/QuestionWebController/editPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
