...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/convenienceTelephone/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 350,
		height : 300,
		href : '${basePath}/web/convenienceTelephone/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 350,
		height : 300,
		href : '${basePath}/web/convenienceTelephone/editPage?id=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	parent.$.messager.confirm('询问', '您是否要删除信息 ', function(b) {
		if (b) {
			...
			if (currentUserId) {
				...
				$.post('${basePath}/web/convenienceTelephone/delete', {
					id : id
				}, function(result) {
