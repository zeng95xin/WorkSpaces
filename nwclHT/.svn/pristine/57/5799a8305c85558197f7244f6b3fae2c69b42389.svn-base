   ...
   <script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${basePath}/web/QuestionnaireManagerWebController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
...
/* 			frozenColumns : [ [  ] ], */
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					var str = '&nbsp;&nbsp;&nbsp;&nbsp;';
					str += $.formatString('<img onclick="questionFun(\'{0}\');" src="{1}" title="问题"/>', row.id, '${basePath}/style/images/extjs_icons/eye.png');
					str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
			if (b) {
					...
					$.post('${basePath}/web/QuestionnaireManagerWebController/delete', {
						id : id
					}, function(result) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${basePath}/web/QuestionnaireManagerWebController/batchDelete', {
						ids : ids.join(',')
					}, function(result) {
						if (result.code=="SUCCESS") {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 700,
			height : 300,
			href : '${basePath}/web/QuestionnaireManagerWebController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 700,
			height : 300,
			href : '${basePath}/web/QuestionnaireManagerWebController/editPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function questionFun(id) {
		window.open(
			'${basePath}/web/QuestionWebController/questionPage?questionId=' + id, window,
...
<body>
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="width: 100%">
					<tr>
						...
						<td>是否解决</td>
						<td>
							<select  style="width: 100px" name="status" class="easyui-combobox">
						...
						<td>查询选项</td>
						<td>
						<select name=option style="width: 100px"  class="easyui-combobox">
							<option value="-1">--请选择--</option>
	...
	<div id="menu" class="easyui-menu" style="width: 120px; display: none;">
		...
		<div onclick="questionFun();" data-options="iconCls:'eye'">问题</div>
	</div>
</body>
</html>
