...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/bjwdt/payment/dataGrid?ownerCode=${ownerCode}',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
		...
		nowrap : false,
		columns : [ [ 
 			{field : 'paymentType', title : '类型', width : 150, align : 'center', sortable : false }
 			,{field : 'paymentNum', title : '费用', width : 150}
 			,{field : 'paymentTime', title : '时间', width : 150}
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${basePath}/web/bjwdt/payment/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${basePath}/web/bjwdt/payment/editPage?code=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除 ', function(b) {
		if (b) {
			...
			$.post('${basePath}/web/bjwdt/payment/delete', {
				code : id
			}, function(result) {
