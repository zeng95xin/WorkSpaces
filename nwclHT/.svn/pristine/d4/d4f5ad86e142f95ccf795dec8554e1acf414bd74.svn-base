...
$(function() {
	dataGrid = $('#dataGrid').datagrid({
		url : '${basePath}/web/bjwdt/version/dataGrid',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
...
function  addFun(){
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${basePath}/web/bjwdt/version/addPage',
		buttons : [{
				text : '添加',
				handler : function(){
...
function editFun(id) {
	...
	parent.$.modalDialog({
		...
		width : 600,
		height : 500,
		href : '${basePath}/web/bjwdt/version/editPage?code=' + id,
		buttons : [ {
			text : '保存',
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
function deleteFun(id) {
	...
	parent.$.messager.confirm('询问', '您是否要删除 ', function(b) {
		if (b) {
			...
			$.post('${basePath}/web/bjwdt/version/delete', {
				code : id
			}, function(result) {
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="width:900px;">
					<tr>
						...
						<th>类型</th>
						<td><select name="typeString" class=" span2" onchange="searchFun()" >
