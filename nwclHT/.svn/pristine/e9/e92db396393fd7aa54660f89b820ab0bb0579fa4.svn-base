...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid(
			{
			url : '${basePath}/web/articleRelease/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [
				{
					...
					formatter : function(value, row, index) {
						if(value != null){
						var aa = '<a href="#" onclick="imgFun(\''+ row.id+ '\')">'
				...
				formatter : function(value, row, index) {
					var str = '&nbsp;&nbsp;&nbsp;&nbsp;';
				    str += $.formatString('<img onclick="imgFun(\'{0}\');" src="{1}" title="查看"/>', row.id, '${basePath}/style/images/extjs_icons/eye.png');
					str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	/*function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除信息 ', function(b) {
			if (b) {
				...
				$.post('${basePath}/web/articleRelease/delete', {
					id : id
				}, function(result) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${basePath}/web/articleRelease/batchDelete', {
						ids : ids.join(',')
					}, function(result) {
						if (result.code == "SUCCESS") {
	...
	function imgFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 800,
			height : 700,
			href : '${basePath}/web/articleRelease/imgPage?id=' + id,
		});
	}

	/* function eyeFun(id) {
		window.open(
			'${basePath}/web/articleRelease/detailManager?id=' + id, window,
					...
					<tr>
						...
						<td><select name="option" style="width: 100px"
							...
							data-options="editable:false,width:120,panelHeight:'auto'">
								...
								<option value="1">放行物品</option>
								<option value="2">放行时间</option>
								<option value="3">业主姓名</option>
								<option value="4">业主电话</option>
						</select></td>
						<td>关键字</td>
